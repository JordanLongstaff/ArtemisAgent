...
class ValueIntPacketTest : DescribeSpec({
    describe("ValueIntPacket") {
        describe("Subtypes") {
            ...
            withData<Triple<KClass<out ArtemisPacket>, Byte, Byte>>(
                nameFn = {
                ...
                val anno = it.first.findAnnotation<Packet>().shouldNotBeNull()
                anno.subtype shouldBeEqual it.third
            }

            it("Throws if subtype is missing") {
                @Packet(
                    origin = Origin.CLIENT,
                    hash = TestPacketTypes.VALUE_INT,
                )
                class InvalidPacket : ValueIntPacket(0)
