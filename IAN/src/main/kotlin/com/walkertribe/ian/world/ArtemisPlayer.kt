 ...
 * @author dhleong
 */
class ArtemisPlayer(id: Int, timestamp: Long) : BaseArtemisShip(id, timestamp) {
    /**
    ...
    override fun checkType(other: ArtemisObject) {
        require(other is ArtemisPlayer) { "Player ship can only update other player ships" }
    }
    ...
    private infix fun updatesPlayer(plr: ArtemisPlayer) {
        updateProp(plr, ArtemisPlayer::alertStatus)
        updateProp(plr, ArtemisPlayer::shipIndex)
        updateProp(plr, ArtemisPlayer::capitalShipID)
        updateProp(plr, ArtemisPlayer::warp)
        updateProp(plr, ArtemisPlayer::driveType)
        updateProp(plr, ArtemisPlayer::dockingBase) {
    ...
    private infix fun updatesUpgradesFor(plr: ArtemisPlayer) {
        updateProp(plr, ArtemisPlayer::doubleAgentActive)
        updateProp(plr, ArtemisPlayer::doubleAgentCount)
        updateProp(plr, ArtemisPlayer::doubleAgentSecondsLeft)
    }

    internal object PlayerDsl : Dsl<ArtemisPlayer>(ArtemisPlayer::class) {
    ...
    internal object WeaponsDsl : Dsl<ArtemisPlayer>(ArtemisPlayer::class) {
    ...
    internal object UpgradesDsl : Dsl<ArtemisPlayer>(ArtemisPlayer::class) {
