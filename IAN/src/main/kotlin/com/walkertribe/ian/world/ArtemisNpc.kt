 ...
 * @author dhleong
 */
class ArtemisNpc(id: Int, timestamp: Long) : BaseArtemisShip(id, timestamp) {
    /**
    ...
    fun hasBeenScannedBy(ship: BaseArtemisShip): BoolState =
        hasBeenScannedBy(ship.side.value)

    override val type = ObjectType.NPC_SHIP

    override fun checkType(other: ArtemisObject) {
        require(other is ArtemisNpc) { "NPC ship can only update other NPC ships" }
    }
    ...
    override fun updates(other: ArtemisObject) {
        ...
        if (other is ArtemisNpc) {
            updateProp(other, ArtemisNpc::isEnemy)
            updateProp(other, ArtemisNpc::isSurrendered)
            updateProp(other, ArtemisNpc::isInNebula)
            updateProp(other, ArtemisNpc::scanBits)
        }
    }

    internal object Dsl : BaseArtemisShip.Dsl<ArtemisNpc>(ArtemisNpc::class) {
