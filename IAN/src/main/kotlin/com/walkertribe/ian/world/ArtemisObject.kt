 ...
 * @author dhleong
 */
interface ArtemisObject : ListenerArgument {
    /**
     ...
     * method will throw an [IllegalStateException] if either object's hasPosition
     * would return false. If the Y coordinate for an object is undefined, 0 is
     * assumed.
     *
     * The calculation is simply sqrt(distanceSquaredTo(other)).
     */
    infix fun distanceTo(other: ArtemisObject): Float = sqrt(distanceSquaredTo(other))

    /**
     * Returns the square of the distance between this object and the given
     * object. This method will throw an [IllegalStateException] if either object's
     * hasPosition would return false. If the Y coordinate for an object is
     * undefined, 0 is assumed.
     *
     * This function neglects a square-root operation required to calculate
     * the distance, making it a more efficient calculation. Use distanceTo
     * if you actually need the distance instead of its square.
     */
    infix fun distanceSquaredTo(other: ArtemisObject): Float

    /**
     * Same as distanceTo(ArtemisObject), but ignores the Y-axis, giving the
     * distance between the objects from the top-down perspective.
     */
    infix fun horizontalDistanceTo(other: ArtemisObject): Float = sqrt(horizontalDistanceSquaredTo(other))

    /**
     * Same as distanceSquaredTo(ArtemisObject), but ignores the Y-axis,
     * giving the square of the distance between the objects from the
    ...
    /**
     * Updates the given object's properties to match any updates provided by this
     * object. If any property of this object is unspecified, the given object's
     * corresponding property will not be updated.
     */
    infix fun updates(other: ArtemisObject)
}
