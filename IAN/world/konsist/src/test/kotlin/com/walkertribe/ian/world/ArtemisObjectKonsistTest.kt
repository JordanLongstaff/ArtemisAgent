...
class ArtemisObjectKonsistTest :
    DescribeSpec({
            ...
            withData(nameFn = { it.name }, objectClasses) { cls ->
                cls.assertTrue {
                    it.primaryConstructor?.takeIf { constr ->
                        constr.parameters.map { param -> param.name to param.type.name } ==
                            requiredParameters
                    } != null
                }
            }
        ...
        describe("ArtemisPlayer") {
            ...
            it("Dsl inner class is sealed with private constructor") {
                dslClasses.shouldNotBeEmpty()
            }
            ...
            describe("Dsl subclasses are all data objects") {
                ...
                withData(nameFn = { it.name }, dslClasses.objects()) { dslObject ->
                    dslObject.assertTrue { dsl ->
                        dsl.hasDataModifier && dsl.hasParentWithName("Dsl")
                    }
                }
            }
        }
