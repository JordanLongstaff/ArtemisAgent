...
class ConnectFragmentTest : TestCase() {
                    ...
                    ConnectPageScreen {
                        ...
                        if (!isEmulator || Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                            ...
                            step("Wait for scan to finish") {
                                Screen.idle(scanTimeout.seconds.inWholeMilliseconds)
                            }
                        }
    ...
    fun connectionSuccessTest() {
        run {
            mainScreenTest(false) {
                withViewModel { viewModel ->
                    scenario(ConnectScenario(FAKE_SERVER_IP, viewModel))
    ...
    fun connectionFailedTest() {
        run {
            mainScreenTest {
                withViewModel { viewModel ->
                    scenario(ConnectScenario("127.0.0.1", viewModel))
            ...
            mainScreenTest {
                withViewModel { viewModel ->
                    ...
                    runTest(timeout = 2.minutes) {
                        ...
                        booleanArrayOf(showingInfo, !showingInfo, showingInfo).forEachIndexed {
                            index,
                            showing ->
