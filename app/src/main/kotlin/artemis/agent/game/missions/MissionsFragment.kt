        ...
        fun bind(entry: SideMissionEntry) {
            with(entryBinding) {
                ...
                val rewardList = viewModel.displayedRewards.filter {
                    ...
                    val value = entry.rewards[it.ordinal] * if (it == RewardType.NUKE) 2 else 1
                    val name =
                        if (it == RewardType.PRODUCTION) entry.destination.obj.nameString else null
                    val prefix = name?.let { n -> "$n " } ?: ""
                    val suffix = if (value > 1) " x$value" else ""
                    "$prefix${labels[it.ordinal]}$suffix"
                }
